...
def unpack_from(fmt, file_handle):
    size = struct.calcsize(fmt)
    return struct.unpack(fmt, file_handle.read(size))
...
parser.add_argument(
    "-r", "--rootdir", type=str,
    help="scan the output files to obtain a list of files with unnamed symbols "
         "(note: will rebuild objects as necessary)")
parser.add_argument(
    "-l", "--list", type=int, default=0,
...
if args.rootdir:
    for line in (
        subprocess.Popen(
            ["make", "-C", args.rootdir, "-s", "-p", "DEBUG=1"],
