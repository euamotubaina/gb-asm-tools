...
def rows_to_tiles(rows, width, height):
    ...
    yield from (
        tuple(tuple(row[x: x + 8]) for row in rows[y: y + 8]) for y in range(0, height, 8) for x in range(0, width, 8)
    )
...
def tiles_to_rows(tiles, width, height):
    ...
    yield from (
        [tile[y][x] for tile in tile_row for x in range(8)] for tile_row in tile_rows for y in range(8)
    )
...
def main():
    ...
    while True:
        if len(sys.argv) < 2:
            print(f"Usage: {sys.argv[0]} [-f|--flip] [-x|--cross] image.png...", file=sys.stderr)
            sys.exit(1)
